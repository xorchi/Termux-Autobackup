#!/usr/bin/env python3
"""
bip39cli.py - BIP39 Mnemonic Tool
Zero-dependency, Python 3 stdlib only.
Terverifikasi terhadap Coinomi bip39-coinomi source.

  generate  validate  info  derive  monero
"""
...
def _ed_base():
    y=4*pow(5,-1,_ED_P)%_ED_P; x=_ed_recover_x(y)
...
def _slip10_child(k,c,idx):
    # SLIP-0010 Ed25519: selalu hardened
...
def _ed_pubkey(priv32):
    ...
    x,y,z,_=R; zi=pow(z,-1,_ED_P)
    ax=x*zi%_ED_P; ay=y*zi%_ED_P
...
def _keccak256(data):
    ...
    for i in range(0,len(msg),136):
        for j in range(17):
            xj,yj=j%5,j//5
            A[xj][yj]^=int.from_bytes(msg[i+j*8:i+j*8+8],'little')
        for r in range(24):
...
def _wif(p):     return _b58chk(b'\x80'+p+b'\x01')
...
def _bech32(hrp,prog,witver=0):
    ...
    ex=[ord(x)>>5 for x in hrp]+[0]+[ord(x)&31 for x in hrp]
    def pm(v):
...
def _p2wpkh(pub): return _bech32('bc',_h160(pub))
...
def _bnb_addr(priv32):
    # BNB BEP2: bech32 dengan hrp "bnb", program = hash160(pub)
    pub=_pub(int.from_bytes(priv32,'big'))
    return _bech32('bnb',_h160(pub))

def _sol_addr(priv32):
    # Solana: base58 dari Ed25519 public key (32 byte)
    return _b58_plain(_ed_pubkey(priv32))
...
def _xpub(k,c,ver=b'\x04\x88\xB2\x1E'):
    return _b58chk(ver+b'\x00'+b'\x00'*4+b'\x00'*4+c+_pub(int.from_bytes(k,'big')))
def _zpub(k,c): return _xpub(k,c,ver=b'\x04\xb2\x47\x46')

# ── Monero ───────────────────────────────────────────────
_L = 2**252+27742317777372353535851937790883648493

def _sc_reduce32(b): return (int.from_bytes(b,'little')%_L).to_bytes(32,'little')
def _hash_to_scalar(b): return _sc_reduce32(_keccak256(b))

def _monero_keys(seed,path="m/44'/128'/0'/0/0"):
    # Coinomi: BIP32 private key → keccak256 → sc_reduce32 = spend_sec
    k,c=_derive(seed,path)
    spend_sec=_sc_reduce32(_keccak256(k))
    view_sec=_hash_to_scalar(spend_sec)
    return spend_sec, view_sec

def _ed25519_pub_monero(scalar32):
    # Monero pakai Ed25519 langsung dari scalar (bukan hashed)
    # berbeda dengan SOL yang hash dulu
    a=int.from_bytes(scalar32,'little')
    B=_ed_base(); R=_ed_mul(a,B)
    x,y,z,_=R; zi=pow(z,-1,_ED_P)
    ax=x*zi%_ED_P; ay=y*zi%_ED_P
    out=bytearray(ay.to_bytes(32,'little')); out[31]|=(ax&1)<<7
    return bytes(out)

def _monero_addr(spend_pub, view_pub, network=0x12):
    # Standard address: network byte + spend_pub + view_pub + checksum(4 bytes keccak)
    data=bytes([network])+spend_pub+view_pub
    chk=_keccak256(data)[:4]
    return _b58_plain(data+chk)

def _encode_monero25(spend32):
    WC=_MONERO_WC; idxs=[]
    for i in range(8):
...
def _generate(wc=12):
    eb=_WC.get(wc)
    if not eb: raise ValueError("Kata harus 12/15/18/21/24")
...
def _validate(phrase):
    ws=phrase.strip().lower().split()
    if len(ws) not in _WC: return False,f"Jumlah kata tidak valid: {len(ws)}"
    for w in ws:
        if w not in _BIP39_IDX: return False,f"Kata tidak dikenal: '{w}'"
    ...
    h=hashlib.sha256(ent).digest()
    for i in range(cs):
        if bits[eb+i]!=(h[i//8]>>(7-i%8))&1: return False,"Checksum tidak valid"
    return True,"Mnemonic valid"
...
def cmd_generate(args):
    ...
    if wc not in _WC: sys.exit("Error: gunakan 12/15/18/21/24")
    phrase=_generate(wc)
    _hr('MNEMONIC BARU')
    print(phrase)
    print(f"\n({wc} kata | {_WC[wc]} bit)")
...
def cmd_validate(args):
    if not args: sys.exit("Error: berikan mnemonic")
    ok,msg=_validate(args[0])
    _hr('VALIDASI')
    print(f"  Status: {msg}")
    if ok:
        ws=args[0].split()
        print(f"  Kata  : {len(ws)} | {_WC[len(ws)]} bit")
    _hr()
...
def cmd_info(args):
    if not args: sys.exit("Error: berikan mnemonic")
    phrase=args[0]; pp=args[1] if len(args)>1 else ''
    ok,msg=_validate(phrase)
    if not ok: sys.exit(f"Error: {msg}")
    sd=_seed(phrase,pp)
    ...
    print(f"  Status    : {msg}")
    print(f"  Kata      : {len(ws)} | {_WC[len(ws)]} bit")
    print(f"  Passphrase: {repr(pp) if pp else '(kosong)'}")
    print()
    print(f"  Entropy   : {_ent_hex(phrase)}")
    print(f"  Seed      : {sd.hex()}")
    print()
    xprv_p=b'\x04\x88\xAD\xE4'+b'\x00'+b'\x00'*4+b'\x00'*4+cr+b'\x00'+kr
    print(f"  Root xprv : {_b58chk(xprv_p)}")
    print(f"  Root xpub : {_xpub(kr,cr)}")
    _hr()

# ── subcommand: derive ───────────────────────────────────
def cmd_derive(args):
    if not args: sys.exit("Error: berikan mnemonic")
    phrase=args[0]; pp=''; coin='btc'; count=5; mode='all'; cpath=''
    i=1
    ...
    while i<len(args):
        ...
        elif a=='--path'  and i+1<len(args): cpath=args[i+1]; i+=2
        else: i+=1
    ok,msg=_validate(phrase)
    if not ok: sys.exit(f"Error: {msg}")
    sd=_seed(phrase,pp)
    ...
    def show_secp(base,label,afn,xfn=None):
        k,c=_derive(sd,base)
        ek,ec=_child(k,c,0)
        _hr(label)
        ser=(xfn or _xpub)(ek,ec)
        print(f"  xpub:\n  {ser}\n")
        for j in range(count):
            ck,_=_child(ek,ec,j)
            pub=_pub(int.from_bytes(ck,'big'))
            print(f"  [{j}] {base}/0/{j}")
            print(f"  Alamat : {afn(pub)}")
            print(f"  WIF    : {_wif(ck)}")
            print()

    def show_eth_like(base,label,afn):
        k,c=_derive(sd,base)
        ek,ec=_child(k,c,0)
        _hr(label)
        print(f"  xpub:\n  {_xpub(ek,ec)}\n")
        for j in range(count):
            ...
            pub=_pub(int.from_bytes(ck,'big'))
            print(f"  [{j}] {base}/0/{j}")
            print(f"  Alamat : {afn(ck)}")
            print(f"  Pubkey : {pub.hex()}")
            print(f"  Privkey: 0x{ck.hex()}")
            print()

    def show_sol(base,label):
        _hr(label)
        for j in range(count):
            k,c=_slip10_derive(sd,f"{base}/{j}'")
            pub=_ed_pubkey(k)
            print(f"  [{j}] {base}/{j}'")
            print(f"  Alamat : {_b58_plain(pub)}")
            print(f"  Privkey: {_b58_plain(k+pub)}")
            print()

    if cpath:
        k,c=_derive(sd,cpath)
        _hr(f'Path kustom')
        print(f"  {cpath}\n")
        for j in range(count):
            ck,_=_child(k,c,j)
            pub=_pub(int.from_bytes(ck,'big'))
            print(f"  [{j}] {cpath}/{j}")
            print(f"  Alamat : {_p2wpkh(pub)}")
            print(f"  WIF    : {_wif(ck)}")
            print()
        _hr(); return

    if coin=='btc':
        if mode in ('all','segwit'):
            show_secp("m/84'/0'/0'","BIP84 (bc1q)",_p2wpkh,_zpub)
        if mode in ('all','wrapped'):
            show_secp("m/49'/0'/0'","BIP49 (3...)",_p2sh)
        if mode in ('all','legacy'):
            show_secp("m/44'/0'/0'","BIP44 (1...)",_p2pkh)
    elif coin=='eth':
        show_eth_like("m/44'/60'/0'","Ethereum",_eth_addr)
    elif coin=='bnb':
        show_eth_like("m/44'/60'/0'","BNB BEP2",_bnb_addr)
    elif coin=='sol':
        show_sol("m/44'/501'","Solana")
    elif coin=='xmr':
        # Monero: tampilkan semua 4 key + address per index
        _hr("Monero")
        for j in range(count):
            path=f"m/44'/128'/0'/0/{j}"
            spend_sec,view_sec=_monero_keys(sd,path)
            spend_pub=_ed25519_pub_monero(spend_sec)
            view_pub=_ed25519_pub_monero(view_sec)
            addr=_monero_addr(spend_pub,view_pub)
            print(f"  [{j}] {path}")
            print(f"  Address       : {addr}")
            print(f"  Spend sec     : {spend_sec.hex()}")
            print(f"  Spend pub     : {spend_pub.hex()}")
            print(f"  View sec      : {view_sec.hex()}")
            print(f"  View pub      : {view_pub.hex()}")
            print(f"  Seed 25 kata  :")
            print(f"  {_encode_monero25(spend_sec)}")
            print()
    else:
        sys.exit(f"Error: coin tidak dikenal '{coin}'")
    _hr()

# ── subcommand: monero ───────────────────────────────────
def cmd_monero(args):
    if not args: sys.exit("Error: berikan mnemonic BIP39")
    phrase=args[0]
    ok,msg=_validate(phrase)
    if not ok: sys.exit(f"Error: {msg}")
    sd=_seed(phrase)
    spend_sec,view_sec=_monero_keys(sd)
    spend_pub=_ed25519_pub_monero(spend_sec)
    view_pub=_ed25519_pub_monero(view_sec)
    addr=_monero_addr(spend_pub,view_pub)
    _hr('MONERO')
...
Subcommand:
  ...
  derive "<mnemonic>" [passphrase]
    ...
    --count N         (default: 5)
    --mode all|segwit|wrapped|legacy
    --path "m/84'/0'/0'"

  monero "<mnemonic>"
    4 key + address + 25 kata Monero.
    (kompatibel dengan Coinomi)

Contoh:
  bip39 generate 24
  bip39 validate "..."
  bip39 info "..." ""
  bip39 derive "..." "" --coin btc
  bip39 derive "..." "" --coin eth
  bip39 derive "..." "" --coin sol
  bip39 derive "..." "" --coin xmr --count 1
  bip39 monero "..."

PERINGATAN: Gunakan hanya di perangkat
offline dan terpercaya.
"""
...
if __name__=='__main__':
    ...
    if   sub=='generate': cmd_generate(rest)
    elif sub=='validate': cmd_validate(rest)
    elif sub=='info':     cmd_info(rest)
    elif sub=='derive':   cmd_derive(rest)
    elif sub=='monero':   cmd_monero(rest)
    else: sys.exit(f"Error: tidak dikenal '{sub}'")
